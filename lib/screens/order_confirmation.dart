import 'package:flutter/material.dart';
  ...
  class _OrderConfirmationScreenState extends State<OrderConfirmationScreen> {
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '${cartItem.drinksItem.price.toStringAsFixed(2)} EGP',
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '${cartItem.foodItem.price.toStringAsFixed(2)} EGP',
                    style: const TextStyle(fontSize: 14, color: Colors.black87),
                  ),
                  const SizedBox(height: 3),
                  Text(
    ...
    void _confirmOrder() async {
      // Use the current selected dining option (fallback to provided)
      final currentDining = _selectedDiningOption ?? widget.diningOption;
      final subtotal = CartManager.totalAmount;
      final tax = currentDining == 'Take Away' ? 0.0 : subtotal * 0.14;
      final finalTotal = subtotal + tax;

      // Build items list from both food and drink cart items
      final List<Map<String, dynamic>> items = [
        ...CartManager.cartItems.map((ci) => {
              'id': ci.foodItem.id,
        ...
        ...CartManager.cartDrinkItems.map((ci) => {
              'id': ci.drinksItem.id,
              'name': ci.drinksItem.name,
              'qty': ci.quantity,
              'price': ci.drinksItem.price,
              'total': ci.totalPrice,
            }),
      ];
      ...
      // Ask for confirmation and save when confirmed
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Service Type: $currentDining'),
                const SizedBox(height: 8),
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  // Try to save order to Firestore; if it fails, show a message but continue to clear cart
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Warning: order not saved to server.')),
                    );
                  }
